...
class ZwapOpenToTag extends StatefulWidget implements ResponsiveWidget {
  ...
  ZwapOpenToTag({Key? key, required this.openToTagText, required this.tagIcon, this.callBackClick, this.isClickAble = true, this.isLeft = true}) : super(key: key);
...
class _ZwapOpenToTagState extends State<ZwapOpenToTag> {
  ...
  Widget _getChildWidget() {
    return Container(
        decoration: BoxDecoration(color: !this._isHovered ? this.defaultBackgroundColor : this.hoverBackgroundColor, borderRadius: BorderRadius.all(Radius.circular(ZwapRadius.defaultRadius))),
        child: Row(
